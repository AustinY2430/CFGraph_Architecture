...
module MPU #(
	...
	parameter UpdateWidth  = 64   // Input message width
)(
	...
	//----------------------Update to MGU----------------------//
	output reg [VPropWidth+EIndexWidth+EDegreeWidth-1:0] FIFO_data, // {VertexProp, Edge Addr, Edge Range}
	output reg                   FIFO_ready, // MGU data is ready for MGU to ready
	input                        FIFO_full   // MGU response
);
...
wire  [EIndexWidth-1:0] edge_index;      // Base address of edge
wire [EDegreeWidth-1:0] edge_degree;     // Number of edges = edge degree
reg  [EDegreeWidth-1:0] old_degree;      // Old Edge Degree
reg                     start_send;      // start sending data to MGU
reg                     HALT;


assign edge_index  = store_read_data[(EIndexWidth+EDegreeWidth-1):(EDegreeWidth)];
...
begin
	...
	begin
		if(update_ready && HALT) // Update is ready to read
		begin
	...
	begin
		new_value           <= {1'b0, update_reg[UpdateWidth-AddrWidth-1:0]}; // New Vertex Value
...
begin 
    ...
    FIFO_WAIT:
    begin
        if(start_send && ~FIFO_full)
        begin
            FIFO_data  <= {result, edge_index, edge_degree};
            FIFO_ready <= 1'b1;
            state_a    <= FIFO_SEND;
        end
        else if (start_send)
