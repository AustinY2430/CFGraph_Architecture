...
begin
    ...
    m_axi_ARLEN    <= burst; // length = burst - 1, 1 burst = 1 read
    m_axi_ARSIZE   <= (DATA_WIDTH == 64)? 3'b011: ((DATA_WIDTH == 128)? 3'b100: ((DATA_WIDTH == 256)? 3'b101: 3'b110)); // 64, 128, 256, or 512 bits. Default == 512.
...
assign end_of_read   = m_axi_RVALID && m_axi_RREADY && resp;
...
begin
    ...
    case (state)
        ...
        begin
            if (m_axi_RVALID && m_axi_RREADY && resp) 
            begin
                read_data    <= m_axi_RDATA;
            end
            
            if (m_axi_RLAST && m_axi_RVALID && m_axi_RREADY && resp)
            begin
                guard_RREADY <= 1'b0;
                state        <= RD_IDLE;
            end
            else guard_RREADY   <= 1'b1;
        end
